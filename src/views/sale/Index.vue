          ...
          <div class="right">
            ...
            <div class="tian-container" style="height:500px">
              <ScrollBar color="rgba(255,255,255,0.5)">
                <div
                  v-for="item in indexData.sellingProduct"
                  :key="item.productName"
                  class="tian"
                >
                  <span>{{ item.fieldNumber }}</span>
                  <span>{{ item.productNum }}</span>
                  <span>{{ item.soldProductNum }}</span>
                </div>
              </ScrollBar>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      indexData: {
        soldNum: 0, //已售订单总数
        sellingNum: 0, //售卖中订单总数
        sellOutNum: 0, //已售罄产品批数
        announcement: "这是一条公告", //公告
